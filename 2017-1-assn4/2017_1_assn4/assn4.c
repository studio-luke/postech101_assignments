...
int _2_road_image(IMGMAT* imageList[], int index);
...
int main()
{
	...
	while (1) {
		...
		case '1':
			...
			break;
		case '2':
			index = _2_road_image(image, index);
			break;
		case '3':
...
int _2_road_image(IMGMAT* imgList[], int index)
{
	FILE* image;
	char inputName[30];
	int i, j;
	...
	if ((image = fopen(inputName, "r")) == NULL) {
...
int check_input(char *indexchar, int *index, int indexNum)
{
	...
	else if ('0' <= indexchar[1] && indexchar[1] <= '9' && indexchar[2] == '\0')
...
{
	char indexchar[20], newFileName[32];
	...
	if ((img_c = fopen(newFileName, "w")) == NULL)
...
void _6_reduce_image(IMGMAT* imgList[], int indexNum)
{
	...
	if ((img_r = fopen(newFileName, "w")) == NULL)
		printf("[%s]을 생성할 수 없습니다.\n", newFileName);
