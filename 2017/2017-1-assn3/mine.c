...
//  Local Declarations
	...
	int mapSize, mineNumber, maxMineNumber;

//  Statements
	//  Get Data
	printf("Input the size of map (9~20): ");
	scanf("%d", &mapSize); FFLUSH;
	InputValueExamine(mapSize, 9, 20);
	
	maxMineNumber = (mapSize - 1) * (mapSize - 1);
	printf("Input the number of mines (10~%d): ", maxMineNumber);
	scanf("%d", &mineNumber); FFLUSH;
	InputValueExamine(mineNumber, 10, maxMineNumber);
